...
describe('SocialPlannerController', () => {
  ...
  it('sends only the Planner root/revision identities to Approvals using the server-resolved Company Context', async () => {
    const ctx: RequestContext = {
      tenantId: '11111111-1111-4111-8111-111111111111', workspaceId: '22222222-2222-4222-8222-222222222222', userId: '55555555-5555-4555-8555-555555555555',
      managedContext: { productKey: 'social', operatingMode: 'client', clientId: '33333333-3333-4333-8333-333333333333', companyContextId: '44444444-4444-4444-8444-444444444444', managedTenantId: null },
    };
    await controller.sendRevisionForApproval(ctx, '66666666-6666-4666-8666-666666666666', '77777777-7777-4777-8777-777777777777');
    expect(approvalsService.create).toHaveBeenCalledWith(
      expect.objectContaining({ agencyClientId: ctx.managedContext!.clientId, companyContextId: ctx.managedContext!.companyContextId }),
      ctx.userId,
      { subjectType: 'planner_content_revision', subjectId: '66666666-6666-4666-8666-666666666666', subjectRevisionId: '77777777-7777-4777-8777-777777777777' },
    );
  });
