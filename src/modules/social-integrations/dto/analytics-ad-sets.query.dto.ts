 ...
 * `CAMPAIGN_SORT_VALUES`: `class-validator` needs an array at runtime and the
 * type union does not survive compilation.
 */
export const ADSET_SORT_VALUES: readonly SocialAdAdSetSort[] = [
  'spend',
  'impressions',
  'clicks',
  'leads',
...
/**
 * The query of a per-ad-set read.
 *
 * Same two-layer validation as `AnalyticsCampaignsQueryDto`: this DTO gives
 * the caller a 400 naming what is allowed, and the service maps the value
 * again through a closed lookup so nothing reaches the ORDER BY as text.
 */
export class AnalyticsAdSetsQueryDto {
  @IsUUID()
  connectionId!: string;
  ...
  @IsOptional()
  @IsIn(ADSET_SORT_VALUES as string[])
