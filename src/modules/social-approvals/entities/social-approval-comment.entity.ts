...
@Index('IDX_social_approval_comments_request_created', ['approvalRequestId', 'createdAt'])
@Check('CK_social_approval_comments_body', 'btrim("body") <> \'\'')
@Check('CK_social_approval_comments_stage', '"stage" IS NULL OR "stage" IN (\'internal\', \'client\')')
@Check('CK_social_approval_comments_actor', `("actor_type" = 'user' AND "actor_user_id" IS NOT NULL) OR ("actor_type" = 'system' AND "actor_user_id" IS NULL)`)
export class SocialApprovalCommentEntity {
  @PrimaryGeneratedColumn('uuid') id!: string;
  @Column({ name: 'approval_request_id', type: 'uuid' }) approvalRequestId!: string;
  @Column({ type: 'varchar', length: 16, nullable: true }) stage!: SocialApprovalStage | null;
