...
@Index('IDX_social_approval_stage_decisions_request_created', [
  'approvalRequestId',
  'createdAt',
])
@Check(
  'CK_social_approval_stage_decisions_stage',
  "\"stage\" IN ('internal', 'client')",
)
@Check(
  'CK_social_approval_stage_decisions_kind',
  "\"decision\" IN ('approved', 'changes_requested')",
)
@Check(
  'CK_social_approval_stage_decisions_actor',
...
export class SocialApprovalStageDecisionEntity {
  ...
  @Column({ name: 'comment_id', type: 'uuid', nullable: true }) commentId!:
    | string
    | null;
