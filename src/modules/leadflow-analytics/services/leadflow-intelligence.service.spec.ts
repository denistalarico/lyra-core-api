...
describe('LeadFlowIntelligenceService', () => {
  it('isolates Company A recommendations, decisions, and generation uniqueness from Company B', async () => {
    const companyAContext = {
      ...ctx,
      managedContext: {
        productKey: 'leadflow',
        operatingMode: 'client',
        clientId: '70000000-0000-4000-8000-000000000007',
        companyContextId: '80000000-0000-4000-8000-000000000008',
        managedTenantId: null,
      },
    } as const;
    const companyBRecommendation = recommendation();
    companyBRecommendation.contextType = 'client' as never;
    companyBRecommendation.agencyClientId = companyAContext.managedContext.clientId;
    companyBRecommendation.companyContextId = '90000000-0000-4000-8000-000000000009';
    const sample = {
      automationId: companyBRecommendation.targetId,
    ...
    const { service, recommendations, dataSource } = buildService({
      ...
      recommendation: companyBRecommendation,
    });

    await service.list(companyAContext);
    expect(recommendations.find).toHaveBeenCalledWith(expect.objectContaining({
    ...
    await expect(service.decide(companyAContext, companyBRecommendation.id, {
      action: 'reject',
    })).rejects.toBeInstanceOf(NotFoundException);

    await service.generate(companyAContext, {
      from: '2026-07-01T00:00:00.000Z',
      to: '2026-07-30T00:00:00.000Z',
    });
    expect(recommendations.findOne).toHaveBeenCalledWith(expect.objectContaining({
      where: expect.objectContaining({
        agencyClientId: companyAContext.managedContext.clientId,
        companyContextId: companyAContext.managedContext.companyContextId,
      }),
