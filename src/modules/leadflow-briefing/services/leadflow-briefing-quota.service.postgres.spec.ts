...
run('LeadFlowBriefingQuotaService PostgreSQL', () => {
  ...
  beforeAll(async () => {
    ...
    const otherClient = await AgencyDataSource.getRepository(AgencyClient).save(
      {
        tenantId,
        workspaceId,
        displayName: 'Quota alternate client',
      },
    );
    ...
    const source = await AgencyDataSource.getRepository(
      LeadFlowBriefingSourceEntity,
    ).save({
      tenantId,
      workspaceId,
    ...
    const otherSource = await AgencyDataSource.getRepository(
      LeadFlowBriefingSourceEntity,
    ).save({
      tenantId,
      workspaceId,
  ...
  it('allows content that stays under the ceiling', async () => {
    const service = new LeadFlowBriefingQuotaService(
      AgencyDataSource,
      fakeConfig(1000),
    );
    await expect(
      service.assertWithinQuota(ctx(), settingsId, 300),
  ...
  it('rejects content that would cross the ceiling', async () => {
    ...
    await expect(
      service.assertWithinQuota(ctx(), settingsId, 500),
