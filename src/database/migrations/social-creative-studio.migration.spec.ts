...
function collectSql(run: (queryRunner: never) => Promise<void>) {
  const sql: string[] = [];
  const runner = { query: jest.fn((statement: string) => { sql.push(statement); return Promise.resolve(); }) };
...
describe('social creative studio migration', () => {
  it('creates only logical asset tables and keeps the current-version cycle safe', async () => {
    const sql = await collectSql(runner => new CreateSocialCreativeStudio1794100000000().up(runner));
    for (const table of ['social_creative_folders', 'social_creative_assets', 'social_creative_asset_versions']) expect(sql).toContain(`CREATE TABLE IF NOT EXISTS "${table}"`);
  ...
  it('registers entities and migration solely on the agency datasource', () => {
    expect(AgencyDataSource.options.migrations).toContain(CreateSocialCreativeStudio1794100000000);
    for (const entity of [CreativeAssetEntity, CreativeAssetVersionEntity, CreativeFolderEntity]) { expect(agencyEntities).toContain(entity); expect(AgencyDataSource.options.entities).toContain(entity); }
  });
});
