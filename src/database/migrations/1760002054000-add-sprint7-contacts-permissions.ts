...
const CONTACT_PERMISSION_KEYS = [
  ...
  'shared.contacts.import.admin',
  'shared.contacts.export.admin',
  'shared.contacts.merge.admin',
  'shared.contacts.delete.owner_only',
];

export class AddSprint7ContactsPermissions1760002054000
  implements MigrationInterface
{
  name = 'AddSprint7ContactsPermissions1760002054000';

  public async up(queryRunner: QueryRunner): Promise<void> {
    for (const permissionKey of CONTACT_PERMISSION_KEYS) {
    ...
    for (const roleKey of PLATFORM_ROLE_KEYS) {
      ...
      for (const permissionKey of DEFAULT_ROLE_PERMISSION_MATRIX[roleKey]) {
        if (!CONTACT_PERMISSION_KEYS.includes(permissionKey)) {
          continue;
        }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    for (const roleKey of Object.values(PlatformRoleKey)) {
      await queryRunner.query(
        ...
        `,
        [CONTACT_PERMISSION_KEYS, roleKey],
      );
    }

    await queryRunner.query(
      `DELETE FROM platform_permissions WHERE key = ANY($1)`,
      [CONTACT_PERMISSION_KEYS],
    );
  }
}
