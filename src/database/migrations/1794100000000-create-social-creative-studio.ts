...
export class CreateSocialCreativeStudio1794100000000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_social_creative_folders_scope" ON "social_creative_folders" ("tenant_id", "workspace_id", "agency_client_id")`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_social_creative_folders_parent" ON "social_creative_folders" ("parent_id")`,
    );
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_social_creative_assets_scope_created" ON "social_creative_assets" ("tenant_id", "workspace_id", "agency_client_id", "created_at" DESC)`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_social_creative_assets_folder" ON "social_creative_assets" ("folder_id")`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_social_creative_assets_content" ON "social_creative_assets" ("content_item_id")`,
    );
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_social_creative_asset_versions_asset" ON "social_creative_asset_versions" ("creative_asset_id", "version_number" DESC)`,
    );
    // Added after versions to break the only cyclic relationship safely.
    await queryRunner.query(
      `ALTER TABLE "social_creative_assets" ADD CONSTRAINT "FK_social_creative_assets_current_version" FOREIGN KEY ("current_version_id") REFERENCES "social_creative_asset_versions" ("id") ON DELETE RESTRICT`,
    );
  }
  async down(queryRunner: QueryRunner): Promise<void> {
    // The asset points at its current version, so release the cycle before
    // dropping the version table. PostgreSQL refuses the original drop order.
    await queryRunner.query(
      'ALTER TABLE IF EXISTS "social_creative_assets" DROP CONSTRAINT IF EXISTS "FK_social_creative_assets_current_version"',
    );
    await queryRunner.query(
      'DROP TABLE IF EXISTS "social_creative_asset_versions"',
    );
